...
export function saveAnatomyNote(note: { note_id?: string; document_id: string; page: number; line_start?: number; line_end?: number; content: string }) { return fetch(`${baseUrl}/api/anatomy/notes`, { method: 'PUT', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify(note) }).then(async response => { if (!response.ok) throw new Error(await requestError(response)); return response.json() as Promise<AnatomyNote>; }); }
