...
export function editTeacherCaseDraft(draftId: string, payload: { title?: string; department?: string; chief_complaint?: string; difficulty?: string; suspected_diagnosis?: string; present_illness?: string; learning_goal?: string; teacher_note?: string }) {
  return fetch(`${baseUrl}/api/teacher/case-drafts/${encodeURIComponent(draftId)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then((response) => { if (!response.ok) throw new Error(`Request failed: ${response.status}`); return response.json() as Promise<TeacherCaseDraft>; });
}
