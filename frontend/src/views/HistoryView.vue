...
import { ArrowRight, CalendarDays, Search } from '@lucide/vue';
    ...
    <section class="history-timeline">
      <article v-for="item in records" :key="item.id">
        ...
        <div class="history-record">
          ...
          <div class="history-score"><strong>{{ item.score }}</strong><small>综合评分</small></div>
          <div class="history-meta"><span>{{ item.duration }}</span><span>{{ item.improvements.length }} 项改进建议</span></div>
          <button type="button" title="查看训练报告" @click="router.push(`/training-report/${item.id}`)"><ArrowRight :size="18" /></button>
        </div>
      </article>
    </section>
  </div>
