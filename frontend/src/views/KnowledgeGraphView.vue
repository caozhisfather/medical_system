...
import { BookOpenCheck, ChevronRight, Network, PanelRightClose, PanelRightOpen, Search, Sparkles } from '@lucide/vue';
...
const graphDetailOpen = ref(true);
...
<template>
  <div class="workspace-page graph-page">
    ...
    <div class="graph-workbench" :class="{ 'detail-hidden': !graphDetailOpen }">
      <section class="graph-stage">
        <div class="graph-legend"><span v-for="(text, key) in typeLabels" :key="key"><i :class="`node-${key}`"></i>{{ text }}</span></div>
        <button class="graph-detail-toggle" type="button" :title="graphDetailOpen ? '收起节点详情' : '展开节点详情'" @click="graphDetailOpen = !graphDetailOpen"><PanelRightClose v-if="graphDetailOpen" :size="18" /><PanelRightOpen v-else :size="18" />{{ graphDetailOpen ? '收起详情' : '节点详情' }}</button>
