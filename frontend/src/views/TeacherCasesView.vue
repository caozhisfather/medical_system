...
const activeTab = ref<'assets' | 'recommendations'>('assets');
...
<template>
  <div class="workspace-page teacher-case-page">
    ...
    <nav class="teacher-case-tabs" aria-label="病例智能工作台视图">
      <button type="button" :class="{ active: activeTab === 'assets' }" @click="activeTab = 'assets'"><ClipboardCheck :size="17" />病例资产与审核<span>{{ pendingCount }} 待决策</span></button>
    ...
    <section v-if="showBuilder" class="ai-case-builder">
      <header><div><span><Bot :size="20" /></span><div><strong>AI 病例生成任务</strong><small>填写教学意图，Agent 自动补全脚本、评分规则与安全约束</small></div></div><button class="icon-button" type="button" title="关闭" @click="showBuilder = false"><X :size="18" /></button></header>
    ...
    <section v-else class="teacher-recommendation-workspace">
