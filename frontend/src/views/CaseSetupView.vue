...
watch(selectedCaseId, () => {
  if (!variants.value.some((item) => item.variant_id === selectedVariant.value)) {
    selectedVariant.value = variants.value[0]?.variant_id ?? 'A';
  }
});

function nextSetupStep() {
  activeSetupStep.value = Math.min(3, activeSetupStep.value + 1);
}
  ...
  <div class="workspace-page setup-page">
    ...
    <div class="setup-layout">
      <section class="setup-main">
        <nav class="setup-wizard-tabs" aria-label="训练配置步骤">
          <button v-for="step in setupSteps" :key="step.id" type="button" :class="{ active: activeSetupStep === step.id, done: activeSetupStep > step.id }" @click="activeSetupStep = step.id"><span>{{ activeSetupStep > step.id ? '✓' : step.id }}</span>{{ step.label }}</button>
        </nav>
        <div v-show="activeSetupStep === 1" class="setup-section setup-step-panel">
        ...
        <div v-show="activeSetupStep === 2" class="setup-section setup-step-panel">
        ...
        <div v-show="activeSetupStep === 3" class="setup-section setup-step-panel">
      ...
      <aside class="setup-summary">
        ...
        <button class="button-primary" type="button" @click="activeSetupStep < 3 ? nextSetupStep() : start()">{{ activeSetupStep < 3 ? '继续配置' : '进入问诊室' }} <ArrowRight :size="18" /></button>
