...
const state = reactive({
  ...
  history: readStored<TrainingReportRecord[]>('medical_history', initialHistory)
});
...
export const trainingStore = {
  ...
  startSession(caseId: string, difficulty = '标准', mode = '完整训练', variantId = 'A') {
    ...
    const session: TrainingSession = {
      ...
      messages: [
        ...
        { role: 'tutor', content: `当前为${variantFor(caseData, variantId)?.label ?? `病例变体 ${variantId}`}。请围绕“${caseData.chief_complaint}”完成结构化问诊，患者不会主动泄露诊断。` }
      ],
      scores: localScores(caseData, ''),
      missingPoints: localMissing(caseData, ''),
      citations: citationsFor(caseData),
      unlockedExams: [],
      draft: { hypotheses: [], differentials: '', examinations: '', plan: '', evidence: '' }
    };
  ...
  submitSession() {
    ...
    if (session.backendSessionId) {
      void submitTrainingDiagnosis(session.backendSessionId, {
        preliminary_diagnosis: session.draft.hypotheses[0] ?? '',
      ...
      diagnosisPath: [
        ...
        '补充现病史与危险因素',
        `形成假设：${session.draft.hypotheses.join('、') || caseData.hidden_final_diagnosis || '待完善'}`,
