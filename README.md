...
- Markdown 讲解：教材优先回答支持标题、列表、引用和安全 HTML 渲染；涉及结构、位置或临床联系对比时优先生成 Markdown 表格。
- 教材页阅读：教材弹窗直接渲染原始 PDF 页图，支持全屏三维解剖时打开，不因 Fullscreen API 的渲染子树而消失。
- 学生笔记：笔记按“文档 + 页码”分别保存到当前账号，输入后自动保存，不覆盖其它证据页的笔记。

### 二维结构定位与学习档案

- 空间定位测验使用器官或精细结构二维图，不把教材 PDF 页作为答题底图。
- 系统根据当前题目和所选精细结构解析图谱节点与目标区域，点击落在目标结构区内才判为正确。
- 每次提交按账号写入本地 SQLite，保存题目、系统、目标结构、点击坐标、得分、正误和时间。
- 学习档案读取真实测验记录，错题本按题目聚合错误次数、最近错因和最高得分。
...
| `GET` | `/api/anatomy/records` | 获取当前账号的定位测验记录与统计 |
| `GET` | `/api/anatomy/mistakes` | 获取当前账号的错题聚合 |
