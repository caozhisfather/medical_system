...
class ScoringAgent:
    def score(
        ...
        safety_redlines: list[dict[str, str]] = []
        if medication_plan and not any(term in combined for term in ("过敏", "禁忌", "既往用药")):
            safety_redlines.append({"level": "danger", "code": "medication_without_allergy_check", "text": "未核对过敏史或既往用药就提交用药方案"})
        if (preliminary_diagnosis or treatment_principles or medication_plan) and not any(term in combined for term in ("血压", "心率", "血氧", "生命体征", "查体", "体格检查")):
            safety_redlines.append({"level": "danger", "code": "decision_without_vitals", "text": "未完成生命体征或查体评估就进行临床决策"})
        if preliminary_diagnosis and not ordered_tests and not any(term in combined for term in ("检查", "化验", "心电图", "影像")):
            safety_redlines.append({"level": "warning", "code": "diagnosis_without_evidence", "text": "尚未形成检查证据链就提交诊断"})
        if any(term in treatment_principles for term in ("回家", "自行观察", "不用处理")) and case.get("high_risk_misses"):
            safety_redlines.append({"level": "danger", "code": "unsafe_disposition", "text": "存在高风险遗漏时仍建议回家或自行观察"})
        redline_penalty = min(24, sum(8 if item["level"] == "danger" else 4 for item in safety_redlines))
        ...
        dimensions = [
            ...
            self._dimension("教学用药与安全", medication_score, medication_feedback + (f"；安全红线扣分 {redline_penalty} 分" if redline_penalty else "")),
        ...
        total = max(0, round(sum(item["score"] for item in dimensions) / len(dimensions)) - redline_penalty)
        return {
            ...
            "safety_penalty": redline_penalty,
            "feedback": (
